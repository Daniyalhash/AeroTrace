...
export default function BlockC() {
  ...
  const [isCoAValid2, setIsCoAValid2] = useState(null); // null means neither checked
  ...
  const users = userData?.users || []; const [fontsLoaded, setFontsLoaded] = useState(false);
  //important thing
  ...
  const validateForm = () => {
    ...
    if (isPreserved === null || isCoAValid === null || isCoAValid2 === null) {
      return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const newReportRef = ref(database, `/cannibalizationReports/${reportId}/blockC/A`);
      ...
      clearFields();
      // router.push({ pathname: './BlockCb', params: { reportId } }); // Navigate to BlockB
    ...
    try {
      const response = await fetch("https://exp.host/--/api/v2/push/send/", {
        ...
        body: JSON.stringify({
          ...
          body: `Mr. ${selectedUserName} Block C in process, Now Complete Block C Recipient Section,  from ${currentUserEmail}: Click to open the specific form.`,
          data: {
            screen: "BlockCb",
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.formContainer}>
        <Text style={styles.title}>DONOR SECTION ACTIONS</Text>

        <Text style={styles.label}>Last flight operation with the provided component installed revealed no faults including C1F defects:</Text>
        ...
        <Text style={styles.label}>There has been no unusual event that could affect the aircraft component’s serviceability:</Text>
        <View style={styles.checkboxContainer}>
          <Checkbox
            value={isCoAValid2 === true}
            onValueChange={() => setIsCoAValid2(true)}
            color={isCoAValid2 === true ? '#96ACC0' : '#96ACC0'}
          />
          <Text style={styles.checkboxLabel}>Yes</Text>
          <Checkbox
            style={styles.checkboxBox}
            ...
            color={isCoAValid2 === false ? '#96ACC0' : '#96ACC0'}
          />
          <Text style={styles.checkboxLabel}>No</Text>
        </View>

        <Text style={styles.label}>Component has been inspected for satisfactory condition, specifically for damage, corrosion, or leakage and Serviceable tag has been issued:</Text>
        <View style={styles.checkboxContainer}>
          <Checkbox
