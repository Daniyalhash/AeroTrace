...
import{getDatabase, database, push ,onDisconnect ,get,remove } from './firebaseConfig'; // Import Firebase Realtime Database
...
export default function BlockB() {
  ...
  const [donorAircraftRegistration, setDonorAircraftRegistration] = useState('');
  const [remarks, setRemarks] = useState('');
  const [situationroom, setSituationroom] = useState('');
  ...
  const fetchBlockAData = async () => {
    ...
    try {
      ...
      const blockARef = ref(database, `/cannibalizationReports/${reportId}/blockA`);

      // Fetch the data from Firebase
      const snapshot = await get(blockARef);
      if (snapshot.exists()) {
  ...
  const validateForm = () => {
    ...
    const fields = [
      donorAircraftRegistration, remarks, situationroom
    ];
  ...
  useEffect(() => {
    ...
    fetchBlockAData(); // Fetch data whenever the reportId changes
  ...
  try{
    const response = await fetch("https://exp.host/--/api/v2/push/send/", {
      ...
      body: JSON.stringify({
        ...
        body: `Mr. ${selectedUserName} Block B process starts, Now Complete Block B,  from ${currentUserEmail}: Click to open the specific form.`,
        data: {
          screen: "BlockB2",
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.formContainer}>
        <Text style={styles.label}>Donor Aircraft Registration:</Text>
        <TextInput
          style={styles.input}
          value={donorAircraftRegistration}
          onChangeText={(text) => setDonorAircraftRegistration(text)}
          placeholder="Enter the Donor Aircraft Registration No."
          placeholderTextColor="#96ACC0"


        />
        <Text style={styles.label}>Aircraft is adequately preserved, if required:</Text>
        ...
        <View style={styles.checkboxContainer}>
          <Checkbox
            ...
            color={isPreserved === true ? '#96ACC0' : '#96ACC0'} // Customize colors
          ...
          <Checkbox
            ...
            color={isPreserved === false ? '#96ACC0' : '#96ACC0'} // Customize colors
        ...
        <TextInput
          ...
          placeholder="Enter the Remarks (if any) or Type None"
          placeholderTextColor="#96ACC0"

        />
        <Text style={styles.label}>DCE(Situation Room)/EMOD:</Text>
        <TextInput
          style={styles.input}
          value={situationroom}
          onChangeText={(text) => setSituationroom(text)}
          placeholder="Enter DCE(Situation Room)/EMOD:"
          placeholderTextColor="#96ACC0"
...
const styles = StyleSheet.create({
  ...
  googleButton: { backgroundColor: '#4285F4', padding: 10, borderRadius: 5,    marginBottom: 10,
 marginTop: 20 },
