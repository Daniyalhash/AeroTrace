...
export default function BlockB2() {
  ...
  const validateForm = () => {
    const fields = [approval, endorsed];
...
try{
  const response = await fetch("https://exp.host/--/api/v2/push/send/", {
    ...
    body: JSON.stringify({
      ...
      body: `Mr. ${selectedUserName} Block B will ends, Now Complete  Approving Block B ,  from ${currentUserEmail}: Click to open the specific form.`,
      data: {
        screen: "BlockB3",
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.formContainer}>
        ...
        <Text style={styles.label}>Approval Ref No:</Text>
        <TextInput
          style={styles.input}
          value={approval}
          onChangeText={(text) => setApproval(text)}
          placeholder="Enter Approval Ref No"
          placeholderTextColor="#96ACC0"

        />
        <Text style={styles.label}>Endorsed By: DCE Rotable Planning (Signature/Date/Stamp)</Text>
        <TextInput
          style={styles.input}
          value={endorsed}
          onChangeText={(text) => setEndorsed(text)}
          placeholder="Enter Endorsed By: DCE Rotable Planning"
